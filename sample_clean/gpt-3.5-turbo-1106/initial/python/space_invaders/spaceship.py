...
class Spaceship:
    ...
    def new_ship(self):
        ...
        """
        clone_guns = [gun.new_gun() for gun in self._guns]
        shp = Spaceship(clone_guns, self._hp, list(vertex for vertex in self._collider), self._enemy, image=copy.copy(self._image))
        return shp
    ...
    def move(self, vec):
        ...
        self._collider = [(self._image_rect.center[0] + vertex[0], self._image_rect.center[1] + vertex[1]) for vertex in self._collider_relative_vertices]
        for gun in self._guns:
            gun.move(vec)
