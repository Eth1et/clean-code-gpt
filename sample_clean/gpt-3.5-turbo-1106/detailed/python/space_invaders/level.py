...
def go_towards_actions(position, target, speed, fire=False):
    ...
    while abs(position[0] - target[0]) > 0.01 or abs(position[1] - target[1]) > 0.01:
        ...
        def get_ease_and_move(diff_value, speed_value):
            ease = min(1.0, math.log2(abs(diff_value)))
            move = max(diff_value / speed_value, -1.0) if diff_value < 0 else min(diff_value / speed_value, 1.0)
            return ease, move

        ease_x, move_x = get_ease_and_move(diff[0], speed)
        ease_y, move_y = get_ease_and_move(diff[1], speed)
...
# Other phase constant objects remain unchanged
...
# Other level constant objects remain unchanged