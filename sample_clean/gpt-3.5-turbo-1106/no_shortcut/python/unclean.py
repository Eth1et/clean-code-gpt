...
def find_max_value():
    values = np.array([1, -20, 25, 1, -4, 51])
    max_value = max(values[values < 30])
    print(max_value)
    print("hello world")
...
def perform_3d_convolution(size=15, filter_size=3):
    ...
    surf = ax.plot_surface(X, Y, Z, rstride=1, cstride=1, facecolors=cm.coolwarm(convolution_result), antialiased=False)
    ...
    print("Let's go" if input("Enter a message: ") == "I entered a message" else "No way")
