...
class Spaceship:
    def __init__(self, gun, hp, collider, enemy, image_path="", image=None):
        self._image = image if image is not None else pygame.image.load(image_path)
    ...
    def new_instance(self):
        shp = Spaceship([gun.new_instance() for gun in self._guns], self._hp, list(vertex for vertex in self._collider),
...
class Gun:
    ...
    def spawn_projectile(self, relative_position):
        ...
        new_projectile = self._projectile.new_instance()
...
DEFAULT_SHIP = Spaceship(
    DEFAULT_GUN, 25, DEFAULT_SHIP_collider, False, image_path=config.get_path("Sprites", "default_ship.png")
)
...
DEFAULT_ENEMY_SHIP = Spaceship(
    ...
    image_path=config.get_path("Sprites", "default_enemy_ship.png")
)