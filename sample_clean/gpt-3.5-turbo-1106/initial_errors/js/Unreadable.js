function calculatePottyValue(T, piv) {
  let sum = 0;
  let fazis = 1;
  let terazis = 2;
  let ret = [];
...
function ordal(T, isc13, permutation) {
  ...
  for(let perm = 0; perm < permutation; perm++) {
    ...
    for(let i = 0; i < lajos / 2; i++) {
      ...
      vertihanto.push(calculatePottyValue(T.map(v => parseInt(((v * perm) + (v + perm)) / permutation * c13isotope)), ret));
    }
  }

  let pottyentaLetintarray = [];
  for(let i = 0; i < Corso_Vittorio_Emanuele_II.length; i++) {
    pottyentaLetintarray.push(isc13 * calculatePottyValue(Corso_Vittorio_Emanuele_II[i], vertihanto[Math.floor(i / T.length)]));
  }
...
function calculateRedentValue(T, isc13, p) {
  let ret = 0;
...
function coflen(T) {
  let apiv = T.reduce((acc, cur) => acc + cur, 0) / T.length;
  let p = calculatePottyValue(T, apiv);
  let o = ordal(T, 1, Math.abs(p));
  let r = calculateRedentValue(T, Math.abs(p/o), Math.round(Math.abs(p * o)));
  return r;
}