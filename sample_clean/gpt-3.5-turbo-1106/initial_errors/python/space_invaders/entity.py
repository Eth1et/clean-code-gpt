...
from space_invaders.spaceship import new_ship, hp

class Entity(pygame.sprite.Sprite):
    def __init__(self, ship_type, spawn_position, speed, lives=1, looks_down=True):
        super().__init__()
        self._ship = new_ship(ship_type)
        self._hp = hp(ship_type)
        self._max_hp = hp(ship_type)
        self._lives = lives
        self._speed = speed
...
class Enemy(Entity):
    def __init__(self, ship_type, spawn_position, speed, actions):
        super().__init__(ship_type, spawn_position, speed, lives=1, looks_down=True)
    ...
    def execute_next_action(self):
        if len(self._actions) > 0:
            ...
            if action_type == "shoot":
                ...
                self.move(params[0], clamp=False)
            elif action_type == "move_fire":
                self.shoot()
                self.move(params[0], clamp=False)
