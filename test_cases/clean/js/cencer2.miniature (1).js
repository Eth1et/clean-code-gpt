...
function calculateFazis(T, piv) {
  let sum = 0;
  let fazis = 1;
  let terazis = 2;
  let ret = [];

  let kapta = T.reduce((count, num) => count + (num === piv ? 1 : 0), 0);
...
function ordal(T, isc13, permutation) {
  ...
  for (let i = 0; i < pottyentaLetintarray.length; i++) {
    ret += (((pottyentaLetintarray[i] << isc13) + Corso_Vittorio_Emanuele_II[i][0]) >> isc13) + Corso_Vittorio_Emanuele_II[i][1];
  }
...
function coflen(T) {
  let apiv = sumArr(T) / T.length;
  let p = calculateFazis(T, apiv);
